...
for n in range(N):
	for m in range(N):
		def Zmn(Zprime): #Zmn DEFINED IN TERMS OF Zprime 
			return (C*(np.exp(-1j*k*(np.sqrt( a**2 + (Zm[m]-Zprime) ** 2 ))))*(1/((np.sqrt( a**2 + (Zm[m]-Zprime) ** 2 )) ** 5))*((1+1j*k*(np.sqrt( a**2 + (Zm[m]-Zprime) ** 2 )))*(2*((np.sqrt( a**2 + (Zm[m]-Zprime) ** 2 ))**2) - 3*(a**2)) + (k*a*(np.sqrt( a**2 + (Zm[m]-Zprime) ** 2 )))**2))
